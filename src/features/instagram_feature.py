...
class InstagramFeature:
    ...
    def _download_shortcode(self, shortcode: str) -> bool:
        ...
        try:
            ...
            print(f"Instagram Feature: Profile for post {shortcode} does not exist or is private.")
            return False
        except instaloader.exceptions.ConnectionException as e:
            if "Too Many Requests" in str(e) or "429" in str(e):
                print(f"Instagram Feature: Rate limited by Instagram for {shortcode}. Try again later.")
            else:
                print(f"Instagram Feature: Connection error downloading post {shortcode}: {e}")
            return False
        except instaloader.exceptions.PrivateProfileNotFollowedException:
            print(f"Instagram Feature: Cannot download {shortcode}, profile is private and not followed.")
            return False
        except Exception as e:
            print(f"Instagram Feature: An unexpected error occurred while downloading post {shortcode}: {e}")
            return False
    ...
    async def _send_media(self, shortcode: str, original_message: Message):
        ...
        if not os.path.exists(media_dir) or not os.path.isdir(media_dir):
            print(f"Instagram Feature: Media directory '{media_dir}' not found for shortcode {shortcode}.")
        ...
        image_files = sorted([f for f in os.listdir(media_dir) if f.endswith(('.jpg', '.jpeg', '.png'))])
        ...
        if not files_to_upload_paths:
            print(f"Instagram Feature: No suitable media files (mp4, jpg, jpeg, png) found in '{media_dir}' for {shortcode}.")
        ...
        for file_path in files_to_upload_paths:
            try:
                ...
                if e.status == 413: # Payload too large
                    ...
                    print(f"Instagram Feature: Discord HTTP error sending file {file_path}: {e}")
                    await original_message.reply(f"Sorry, I encountered a Discord error trying to send the media for `https://www.instagram.com/p/{shortcode}/`.")
                ...
                await original_message.reply(f"Sorry, I encountered an unexpected error trying to send the media for `https://www.instagram.com/p/{shortcode}/`.")
    ...
    async def on_instagram_message(self, message: Message):
        ...
        print(f"Instagram Feature: Detected Instagram shortcode '{shortcode}' from {message.author.name}")

        status_message = None
        try:
            status_message = await message.channel.send(f"Processing Instagram link for {message.author.mention}...")
            ...
            if not download_successful:
                ...
                if os.path.exists(shortcode): # Cleanup if download failed but created directory
            ...
            print(f"Instagram Feature: General error processing Instagram post {shortcode}: {e}")
            await message.reply(f"Sorry, an unexpected error occurred while processing the Instagram link: `https://www.instagram.com/p/{shortcode}/`.")
